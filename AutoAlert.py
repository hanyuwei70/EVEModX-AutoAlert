...
class AutoAlert(service.Service):
    ...
    __cachemsgs = {}
    __timer = None
    __timer_interval = 3000     # 定时间隔(ms) 不准确
    __isinstant = False         # 是否直接发送
            ...
            if CharID == session.charid:  # 发言的是自己才有效
                if args[0] == ".startalert":
                    ...
                    if self.__alertchannel != 0:
                        sm.GetService('LSC').GetChannelWindow(self.__alertchannel).Speak(
                    ...
                    sm.GetService('LSC').GetChannelWindow(self.__alertchannel).Speak(u"AutoAlert:Alert started",
                                                                                     eve.session.charid,
                                                                                     localEcho=True)  # 本地聊天框刷新
                    if not self.__isinstant:
                        self.__timer=base.AutoTimer(self.__timer_interval,self.__SendMessage) #开启定时器
                    ...
                    if self.__alertchannel == 0:
                        sm.GetService('LSC').GetChannelWindow(self.__alertchannel).Speak(
                            ...
                            localEcho=True)
                        return
                    sm.GetService('LSC').GetChannelWindow(self.__alertchannel).Speak(u"AutoAlert:Alert stopped",
                                                                                     eve.session.charid, localEcho=True)
